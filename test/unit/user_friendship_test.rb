...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  context "a new instance" do
    setup do
      @user_friendship = UserFriendship.new user: users(:conor), friend: users(:mike)
    end
  ...
  # test the method send_request_email ['#' => instance method]
  context "#send_request_email" do
    setup do
      @user_friendship = UserFriendship.create user: users(:conor), friend: users(:mike)
    end
  ...
  context "#mutual_friendship" do
    setup do
      UserFriendship.request users(:conor), users(:jim)
      @friendship1 = users(:conor).user_friendships.where(friend_id: users(:jim).id).first
      @friendship2 = users(:jim).user_friendships.where(friend_id: users(:conor).id).first
    end
  ...
  context "accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:conor), users(:mike)
    end
  ...
  # class method 'request' on UserFriendship class ['.' => class method]
  context ".request" do
    should "create two user friendships" do
      assert_difference 'UserFriendship.count', 2 do
        UserFriendship.request(users(:conor), users(:mike))
      end
    end
  ...
  context "#block!" do
    ...
    should "not allow new requests once blocked" do
      @user_friendship.block!
      uf2 = UserFriendship.request users(:conor), users(:mike)
      assert !uf2.save
    end
  end
